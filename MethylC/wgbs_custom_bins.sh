...
echo 'Bedtools CG ...'
bedtools intersect -sorted -wo -a temp.genome.${bin}bp.sorted.bed -b ${bed}_CG.bed.bismark.cov | groupBy -g 1,2,3 -c 7,8,9 -o mean,sum,sum | awk -v OFS='\t' '{print $1,$2,$3,$4 = ($5 / ($5+$6)*100 ),$5 = ($5 + $6)}' | awk '{ if ($5 >= '$cov') { print } }' > ${bed}_CG_${bin}bp_${cov}cov.bed

echo 'Bedtools CHG ...'
bedtools intersect -sorted -wo -a temp.genome.${bin}bp.sorted.bed -b ${bed}_CHG.bed.bismark.cov | groupBy -g 1,2,3 -c 7,8,9 -o mean,sum,sum | awk -v OFS='\t' '{print $1,$2,$3,$4 = ($5 / ($5+$6)*100 ), $5 = ($5 + $6)}' | awk '{ if ($5 >= '$cov') { print } }' > ${bed}_CHG_${bin}bp_${cov}cov.bed

echo 'Bedtools CHH ...'
bedtools intersect -sorted -wo -a temp.genome.${bin}bp.sorted.bed -b ${bed}_CHH.bed.bismark.cov | groupBy -g 1,2,3 -c 7,8,9 -o mean,sum,sum | awk -v OFS='\t' '{print $1,$2,$3,$4 = ($5 / ($5+$6)*100 ), $5 = ($5 + $6)}' | awk '{ if ($5 >= '$cov') { print } }' > ${bed}_CHH_${bin}bp_${cov}cov.bed
